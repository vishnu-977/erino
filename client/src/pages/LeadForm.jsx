...
export default function LeadForm() {
  ...
  useEffect(()=> {
    ...
    (async ()=> {
      try { 
        ...
        setLead({ ...data, last_activity_at: data.last_activity_at ? data.last_activity_at.slice(0,16) : "" }); 
      } catch(e){ 
        setErr(e.message); 
      }
    })();
  ...
  async function submit(e) {
    ...
    const body = { 
      ...
      lead_value: Number(lead.lead_value),
    ...
    try {
      if (id) {
        ...
        const newLead = await api(`/leads`, { method: "POST", body });
        // Dispatch event to update Leads.jsx
        window.dispatchEvent(new CustomEvent("leadCreated", { detail: newLead }));
      }
      nav("/leads");
    } catch (e) { 
      setErr(e.message); 
